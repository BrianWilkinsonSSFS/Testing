...
public class DayFourPartTwo {
    ...
    public static int callNumbers(ArrayList<BingoCard> bc, int[] calls) {
            ...
            for (BingoCard card : bc) {
                for (int r=0; r<card.getNumRows(); r++) {
                    for (int c=0; c<card.getNumCols(); c++) {
                        if (card.getCard()[r][c].getValue() == calls[i] && !card.getHasWon()) {
                            card.getCard()[r][c].setCalled();
                            if (card.hasBingo())
                                if (!winners.contains(card)) {
                                    card.setWinningNumber(calls[i]);
                                    winners.add(card);
                                }
        ...
        BingoCard last = winners.get(winners.size()-1);
        return last.totalUncalled() * last.getWinningNumber();
    }

import java.awt.*;
import java.io.File;
import java.io.FileNotFoundException;
import java.util.*;

public class DayNinePartTwo {

    private static int[][] grid;
    private static final int NUM_ROWS = 100;
    private static ArrayList<Integer> sizes = new ArrayList<>();

    public static void main(String[] args) throws FileNotFoundException {
        Scanner scan = new Scanner(new File("files/dayNine.txt"));
        String[] rows = new String[NUM_ROWS];
        int row = 0;
        while (scan.hasNextLine()) {
            rows[row] = scan.nextLine();
            row++;
        }
        scan.close();
        grid = new int[NUM_ROWS][rows[0].length()];
        for (int i=0; i<rows.length; i++) {
            for (int j=0; j<rows[i].length(); j++) {
                char current = rows[i].charAt(j);
                grid[i][j] = Integer.parseInt(String.valueOf(current));
            }
        }
        findTotal();
    }

    public static void findTotal() {
        int total = 0;

        for (int row=0; row< grid.length; row++) {
            for (int col=0; col< grid[0].length; col++) {
                if (isLowest(row, col)) {
                    //System.out.println("row: " + row + ", col: " + col + ", value: " + grid[row][col]);
                    int basinSize = getBasinSize(row, col);
                    sizes.add(basinSize);
                }

            }
        }
        Collections.sort(sizes, Collections.reverseOrder());
        System.out.println(sizes.get(0) * sizes.get(1) * sizes.get(2));
    }



    public static boolean isValid(int row, int col) {
        return row >= 0 && row < NUM_ROWS && col >= 0 && col < grid[0].length;
    }

    public static boolean isLowest(int row, int col) {
        int currentVal = grid[row][col];
        if (isValid(row-1, col) && grid[row-1][col]<=currentVal)
            return false;
        if (isValid(row+1, col) && grid[row+1][col]<=currentVal)
            return false;
        if (isValid(row, col-1) && grid[row][col-1]<=currentVal)
            return false;
        if (isValid(row, col+1) && grid[row][col+1]<=currentVal)
            return false;

        return true;
    }

   public static int getBasinSize(int row, int col) {
        int count = 0;
        boolean[][] visited = new boolean[grid.length][grid[0].length];
        Queue<Point> queue = new LinkedList<>();
        queue.add(new Point(row, col));


        while (!queue.isEmpty()) {

            Point p = queue.remove();
            row = p.x;
            col = p.y;

            if (row < 0 || col < 0 || row >= grid.length || col >= grid[0].length ||
                    visited[row][col] || grid[row][col]==9)
                continue;

            visited[row][col] = true;
            count++;
            queue.add(new Point(row, col-1));
            queue.add(new Point(row, col+1));
            queue.add(new Point(row-1, col));
            queue.add(new Point(row+1, col));

        }

        return count;
   }


}
